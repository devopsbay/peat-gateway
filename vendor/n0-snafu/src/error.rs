    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if verb != Verbosity::Full {
            printer = printer.add_frame_filter(Box::new(|frames| {
                // is_dependency_code() was removed in color-backtrace 0.7.3; use remaining
                // frame classification methods to strip noise from backtraces.
                frames.retain(|frame| {
                    !frame.is_post_panic_code() && !frame.is_runtime_init_code()
                })
            }));
        }
